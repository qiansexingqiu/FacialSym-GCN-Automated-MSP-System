...
from dental_segmentator_component import DentalSegComponent
...
comp = DentalSegComponent(
    dataset_name="Dataset112_DentalSegmentator_v100",
    configuration="3d_fullres",
    folds="0",
    nnunet_results="" # setting model weight of nnunet-based DentalSegmentator, Parent directory of the weight file
)
...
input_path = '' # ct_file (dicom directory or nii file)
out_dir = '' # output directory
cut_mm = 0 # Select the number of millimeters to subtract below the CT scan (structures such as the clavicle may affect the model's accuracy).
...
print("STL 文件已生成：", stl_path)

def vtk_smooth_stl(in_stl, out_stl, iterations=30, relaxation=0.5):
    reader = vtk.vtkSTLReader()
...
in_stl = os.path.join(out_dir, patient+'__upper_skull.stl')
out_hi_stl = os.path.join(out_dir, 'origin_upper.stl')
out_small_stl = os.path.join(out_dir, 'down_upper.stl')

hi_stl, small_stl = save_stl_variants(
    in_stl,
    out_hi_stl,
    out_small_stl,
    smooth_iterations=30,
...
in_bone_stl = os.path.join(out_dir, 'bone.stl')
out_bone_hi_stl = os.path.join(out_dir, 'origin_bone.stl')
out_bone_small_stl = os.path.join(out_dir, 'down_bone.stl')
hi_stl_bone, small_stl_bone = save_stl_variants(
    in_bone_stl,
    out_bone_hi_stl,
    out_bone_small_stl,
    smooth_iterations=30,
...
out_stl = os.path.join(out_dir, 'hi_upper.stl')
vtk_smooth_stl(in_stl, out_stl)
...
rgb = np.full_like(upper_points, 128, dtype=int)  # shape: (N, 3)
